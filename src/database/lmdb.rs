...
impl<'a> DatabaseReader for LmdbDatabaseReader<'a> {
    ...
    fn cursor(&self) -> Result<LmdbDatabaseReaderCursor, DatabaseError> {
        ...
        let access = self.txn.access();
        Ok(LmdbDatabaseReaderCursor::new(access, cursor))
    }
...
impl<'a> LmdbDatabaseReaderCursor<'a> {
    pub fn new(access: lmdb::ConstAccessor<'a>, cursor: lmdb::Cursor<'a, 'a>) -> Self {
        Self {
            access,
            cursor,
            started: false,
...
impl<'a> DoubleEndedIterator for LmdbDatabaseReaderCursor<'a> {
    fn next_back(&mut self) -> Option<Self::Item> {
        if !self.started {
            let item = self.seek_last();
            self.started = true;
            item
        } else {
            self.cursor
